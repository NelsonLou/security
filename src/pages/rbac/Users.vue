  ...
  <el-card class="box-card">
    ...
    <div slot="header" class="card-header">
      <wk-form-base inline ref="wk-search" :form-item="data.searchItem" @query="getData">
        ...
        <el-button v-if='permission_List.adminAdd.status' @click="handleAdd" icon="plus" type="primary" class="header-right">添加</el-button>
      </wk-form-base>
    </div>
    ...
    <wk-table-base ref="wk-list" :columns="data.columns" @query="getData">
      <el-table-column label="操作" width="180">
        <template slot-scope="scope">
          <!-- 操作按钮 -->
  				<el-button v-if='permission_List.adminEdit.status' @click="handleEdit(scope.$index, scope.row)" size="small" icon="edit" type="primary">编辑</el-button>
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      ...
      this.data.editItem[2].disabled = true
      this.isEdit = false
      this.$nextTick(() => {
        // 调用组件获取数据
    ...
    handleEdit(index, row) {
      this.dialogVisible()
      this.data.editItem[2].disabled = false
      this.data.editItem[2].disabled = false
      this.editTitle = true
      this.$nextTick(() => {
        // 请求参数
    ...
    editSave(obj) {
      ...
      const para = {
        ...
        admin_password: obj.admin_id ? obj.admin_password : 'dR131^f8',
        info_id: obj.info_id,
